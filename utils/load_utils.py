...
#----------------------------------------------------------------------------
def blend_models(low, high, resolution):
    ...
    for name, param in params_src:
        if not 'mapping' in name:
            if not any(f'synthesis.b{res}' in name for res in resolutions):
                dict_dest[name].data.copy_(param.data)
    model_out.load_state_dict(dict_dest)
    return model_out

def blend_models_stylegan3(low, high, resolution):
    resolutions =  ['input'] + [f"L{i}" for i in range(resolution)]
    ...
    for name, param in params_src:
        if not 'mapping' in name:
            # if not 'affine' in name:
            if not name.startswith('synthesis.input'):
                if not any(name.split("_")[0] == f'synthesis.{res}' for res in resolutions):
                    # print(name)
                    dict_dest[name].data.copy_(param.data)
    model_out.load_state_dict(dict_dest)
    return model_out
...
def convert_tf_generator(tf_G, type="stylegan2"):
    if tf_G.version < 4:
